...
__global__ void spmm_forward_cuda_kernel_with_value(
...
)
{
    ...
    spmm_forward_cuda_kernel_with_value<<<grid, block, dynamic_shared_size>>>(
                                                                    nodePointer.data<int>(),
                                                                    edgeList.data<int>(),
                                                                    blockPartition.data<int>(),
                                                                    edgeToColumn.data<int>(),
...
////////////////////////////////////
/// SPMM forward with CSR values (GCN, GraphSage)
///////////////////////////////////
__global__ void spmm_forward_cuda_kernel_with_value(
	const int * __restrict__ nodePointer,
	const int *__restrict__ edgeList,
	...
	const unsigned nIdx_end = min((bid + 1) * BLK_H, numNodes);
	...
	for (unsigned i = 0; i < num_TC_blocks; i++){
		...
		for (unsigned eIdx = eIdx_start + tid; eIdx < eIdx_end; eIdx += threadPerBlock){
			...
			if (i * BLK_W <= col && col < (i + 1) * BLK_W){
				unsigned row_local = edgeToRow[eIdx] % BLK_H;
				unsigned col_local = col % BLK_W;
				sparse_A[row_local * BLK_W + col_local] = values[eIdx];
